...
\abstract{%
  ...
  To address these challenges, we propose TRACE, a topology-aware visual analytics framework for the evolution of tree structures. TRACE estimates corresponding changes between adjacent trees, thereby representing the evolution as a sequence of local change units for analysis. It further extracts difference-aware substructures that preserve essential context while reducing visual complexity, supporting effective comparison of critical topological changes. TRACE simplifies long sequences based on the distribution of differences to reveal salient intervals, key transition steps, and overall evolutionary trends. Moreover, TRACE supports multi-granularity analysis from local changes to global evolution patterns.

  We evaluate TRACE through case studies on multiple datasets and user interviews. The results show that TRACE helps analysts better understand evolution processes in tree-structured data, locate key structural changes, and identify important patterns and transitions in long sequences.
}
